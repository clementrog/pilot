...
After each step:
- Update STATE.md task.step
- After health: update health.status, health.checked_at
- After commit: update git.head
- After health PASS + commit: update git.lkg
...
imports: "Absolute @/ paths, order: react → next → external → @/ → ./"

typescript:
  strict: true
  any: forbidden
  non_null_assertion: forbidden
...
```yaml
anchoring:
  required: "Unique string anchors"
  forbidden: "Line numbers"
  example: "Find 'export function handlePayment(' and replace"

scope: "Only files in TASK.md scope"
```
